...
function Vente({ setActivePage }) { // ✅ Receive setActivePage prop
  ...
  // ✅ Navigate to new vente page instead of opening modal
  const handleAddNew = () => {
    setActivePage('new-vente'); // Navigate to new vente page
  };

  const handleEdit = (vente) => {
    // You can store the vente to edit in localStorage or context and navigate
    localStorage.setItem('venteToEdit', JSON.stringify(vente));
    setActivePage('new-vente');
  };
  ...
  return (
    <Box sx={{ width: '100%' }}>
      ...
      <Box sx={{ 
        ...
        <Button
          variant="contained"
          onClick={handleAddNew} // ✅ Now navigates to new page
          startIcon={<AddIcon />}
          sx={{
            textTransform: 'none',
