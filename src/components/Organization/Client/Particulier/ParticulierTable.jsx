...
const ParticulierTable = ({ particuliers, onEdit, onDelete }) => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelected = particuliers.map((p) => p.id);
  ...
  const sortedParticuliers = filteredParticuliers.sort(getComparator(order, orderBy));
  const paginatedParticuliers = sortedParticuliers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/particulier/delete/bulk', {
        method: 'DELETE',
      ...
      if (!response.ok) {
        throw new Error('Erreur lors de la suppression des particuliers');
      }

      selected.forEach((id) => {
        const particulierToDelete = particuliers.find((p) => p.id === id);
        if (particulierToDelete) onDelete(particulierToDelete);
      });
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2, borderRadius:'16px' , boxShadow: '0 0 2px 0 rgba(145, 158, 171, 0.2), 0 12px 24px -4px rgba(145, 158, 171, 0.12)' }}>
        ...
        <TableContainer>
          <Table sx={{ minWidth: 650 }}>
            <EnhancedTableHead
              ...
              rowCount={paginatedParticuliers.length}
              numSelected={selected.length}
            />
            <TableBody>
              {paginatedParticuliers.length === 0 ? (
                <TableRow>
        ...
        <TablePagination
          ...
          component="div"
          count={filteredParticuliers.length}
...
ParticulierTable.propTypes = {
  particuliers: PropTypes.array.isRequired,
  onEdit: PropTypes.func.isRequired,
  onDelete: PropTypes.func.isRequired,
};

export default ParticulierTable;